...
def data_to_human_readable(dict_: dict, metadata: dict, cols: Optional[list[int]] = None,
                           key: int = 4) -> dict:
...
def select_from(data_: Tuple[dict, dict], key: str,
    condition_mdata: Callable = (lambda _: True), condition_data: Callable = (lambda _: True),
                condition_key: Callable = (lambda _: True)) -> Tuple[dict, dict]:
